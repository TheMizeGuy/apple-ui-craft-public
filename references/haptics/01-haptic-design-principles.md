...
| Honor system settings | Check `CHHapticEngine.capabilitiesForHardware()`; never force haptics |
...
| Drag snap to grid / position | `.impact(weight: .rigid)` | Communicates alignment |
...
Action significance maps to haptic weight:

| Action significance | Haptic |
|---|---|
| Toggle a small UI option | `.selection` (lightest) |
| Tap an interactive element | `.impact(weight: .light)` |
| Confirm a button press | `.impact(weight: .medium)` |
| Commit a destructive action | `.impact(weight: .heavy)` |
| Snap to grid / alignment | `.impact(weight: .rigid)` |
| Soft elastic settle | `.impact(weight: .soft)` |
...
- Weight communicates significance

| Weight | Feel | Use case |
|---|---|---|
| `.light` | Subtle tap | Toggle switches, minor interactions |
| `.medium` | Moderate tap | Button presses, selections |
...
| Setting | Behavior |
|---|---|
| User has haptics disabled in Settings | All haptics are silently no-ops |
