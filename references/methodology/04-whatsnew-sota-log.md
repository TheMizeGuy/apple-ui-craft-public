...
- **Toolbar overflow control** (the minimize behaviours live in `references/animation/05-gesture-driven.md`; `.visibilityPriority(_:)`, `ToolbarOverflowMenu`, and `.topBarPinnedTrailing` are registry-quarantined and owned by no file yet): `ToolbarMinimizeBehavior.onScrollDown` -- a DIFFERENT type from the 26.0 `TabBarMinimizeBehavior.onScrollDown`; split the gates, never share one `#available` block. Plus `.visibilityPriority(_:)`, `ToolbarOverflowMenu`, `.topBarPinnedTrailing`.
- **Lists and swipe actions escape `List`** (owner `references/interaction/06-custom-controls-reorderable.md`): `.reorderable()` on any `ForEach`, `.reorderContainer(for:)`, `.swipeActions` outside `List`. iOS 26 fallback: `List.onMove` or manual `.draggable` + `.dropDestination`.
- **SwiftData sectioned queries**: `@Query(sort:sectionBy:)` is iOS 27.0+ Beta, NOT iOS 26 -- and never conflate it with Core Data's unrelated iOS-15 `SectionedFetchRequest`, twelve major versions apart. `ResultsSectionCollection`'s exact name is unconfirmed in public docs.
...
- **`AsyncImage`** honors standard HTTP cache headers by default with no code change; `.asyncImageURLSession(_:)` attaches a custom session -- the pre-27 no-cache behaviour is recorded in `references/performance/06-concurrency-ui.md`; the session modifier is owned by no file yet.
