...
| A pulsing "LIVE" badge -- `.symbolEffect(.pulse, options: .repeating)` on a status dot next to "LIVE" text (or any pulse-ring live/online indicator) | Reflexive AI-tell ornament, USER-BANNED by default (2026-07-18): liveness should be content-borne (rows arriving, timestamps updating), and the looping effect additionally needs the double gate (Reduce Motion + Auto-Play Animated Images) it usually ships without | No live badge at all unless the user explicitly requests one; surface connection state only when DEGRADED (quiet "Reconnecting..." text), and let live content prove its own liveness |
