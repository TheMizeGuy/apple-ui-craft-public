...
When the harness announces ultracode, this skill runs conductor-executor: the session model (Fable 5; Opus 4.8 only under Fable fallback) CONDUCTS, and teams of Sonnet 5 executors at `xhigh` effort do the scoped grunt stages. Without ultracode, run the standard architect + accessibility-pass dispatch above unchanged.
